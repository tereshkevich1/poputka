...
class ConsumptionRepositoryImpl @Inject constructor(
    ...
    override suspend fun getForPeriod(start: Long, end: Long): List<Consumption> {
        return dao.getConsumptionsBetween(start, end).map { it.toDomain() }
    }
    ...
    override fun getForPeriodFlow(
        ...
        return dao.getForPeriodFlow(startMillis, endMillis).map { consumption ->
            consumption.map {
                it.toDomain()
            }
        }
    ...
    override suspend fun getTotalVolume(): Int {
        return dao.getTotalVolume() ?: 0
    }

    override suspend fun getTotalVolumeForPeriod(start: Long, end: Long): Int {
        return dao.getTotalVolumeBetween(start, end) ?: 0
    }

    override suspend fun upsert(consumption: Consumption) {
        dao.upsert(consumption.toEntity())
    }
}
